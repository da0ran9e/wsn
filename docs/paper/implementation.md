...
## Program Ancestry Tree (Phả Hệ Chương Trình)

```
uav-example.cc (MAIN ENTRY POINT)
│
├─ NS-3 Framework
│  ├─ core-module
│  ├─ network-module
...
│  └─ energy-module
│  
├─ Ground Nodes (NxN Grid)
│  └─ Each Node:
│     ├─ ns3::Node
│     ├─ Mobility (ConstantPositionMobilityModel)
│     ├─ CellForming (Phase 0 algorithm)
│     ├─ CellFormingMac (extends Cc2420Mac)
│     └─ GroundNodeMac (RX Statistics)
│
├─ UAV Node
│  └─ Contains:
│     ├─ ns3::Node
│     ├─ Mobility (WaypointMobilityModel - circular path)
│     └─ UavMacCc2420 (extends Cc2420Mac for broadcast)
│
└─ Radio Subsystem (CC2420)
   ├─ Cc2420Mac (IEEE 802.15.4 MAC base)
   │  ├─ McpsDataRequest() [TX]
   │  ├─ FrameReceptionCallback() [RX]
   │  └─ CSMA-CA (stub)
   ├─ Cc2420Phy (PHY layer)
   ├─ Cc2420NetDevice (NetDevice wrapper)
   └─ Cc2420EnergyModel (Energy tracking)
```
...
**Proposal**: Tạo file `src/wsn/config/simulation-config.h` để định nghĩa các tham số chung:

```cpp
// Simulation Parameters
#define GRID_SIZE            3          // Ground nodes: GRID_SIZE × GRID_SIZE
#define GRID_SPACING         50.0       // meters
#define CELL_RADIUS          150.0      // meters
#define GRID_OFFSET          1000       // for cell ID calculation

// Timing Parameters (seconds)
#define HELLO_INTERVAL       1.0
#define CL_ELECTION_DELAY    0.5
#define CL_CALCULATION_TIME  2.0
#define UAV_BROADCAST_START  6.0
#define UAV_BROADCAST_INTERVAL 1.0

// Radio Parameters
#define TX_POWER_DBM         10.0
#define RX_SENSITIVITY_DBM   -95.0
#define PAN_ID               0x1234
#define MAC_MIN_BE           3
#define MAC_MAX_BE           5

// UAV Parameters
#define UAV_ALTITUDE         100.0      // meters
#define UAV_SPEED            50.0       // m/s
#define NUM_FRAGMENTS        3
```

**Usage**: Include in `uav-example.cc` để centralize tất cả configuration parameters.

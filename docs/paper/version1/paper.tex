...
\begin{IEEEkeywords}
UAV-assisted dissemination, edge IoT, fragment-based recognition, cooperative detection, mission completion time
\end{IEEEkeywords}
...
%   → tối ưu quỹ đạo tốt, nhưng giả định payload nguyên khối và mô hình kênh đơn giản (in-range / binary),
%     không mô hình hóa việc tích lũy bằng chứng từng phần hay tối ưu time-to-first-detection.
...
%     nhưng không xét đến UAV mobility, quỹ đạo phát tán, hay ràng buộc cửa sổ tiếp xúc.
% Cả hai hướng đều bỏ qua ràng buộc PHY mức gói (A2G propagation, contact-window, SNR→BER→PER)
...
% Mỗi node tích lũy confidence từ các fragment nhận được qua mô hình union probability.
% Intra-cell cooperation được kích hoạt theo ngưỡng trung gian để các node bù fragment còn thiếu
%   trong khi UAV vẫn đang trong vùng — giúp rút ngắn thời gian đến cảnh báo đầu tiên.
% Lớp PHY được mô hình hóa mức gói: A2G propagation theo hình học + contact-window validation
%   + ánh xạ SNR→BER→PER để phản ánh khả năng nhận gói thực tế theo chuyển động UAV.
% Quỹ đạo UAV được tối ưu bằng heuristic GMC (Greedy Max-Coverage with Cost):
%   cân bằng độ phủ mới và chi phí di chuyển qua hàm điểm score = gain / (cost^α + ε),
%   với tập ứng viên gồm vị trí node nghi vấn và centroid k-means tùy chọn.

% Đóng góp chính
% 1. Fragment-based confidence accumulation model kết hợp hợp tác nội ô realtime:
%      node tích lũy evidence từ fragment, hợp tác chia sẻ khi vượt ngưỡng cooperation,
%      cảnh báo khi vượt ngưỡng alert — không cần nhận toàn bộ payload.
% 2. Mobility-coupled packet-level PHY pipeline:
%      A2G path loss theo góc ngẩng, shadowing/fast-fading, contact-window trên airtime gói,
%      và Bernoulli packet drop theo PER — cho đánh giá quỹ đạo sát thực hơn.
% 3. GMC heuristic cho bài toán cân bằng coverage–cost kết hợp cellayer cooperation:
%      greedy waypoint selection với candidate set mở rộng bằng k-means centroid,
%      heuristic thực tế, deterministic, và dễ tích hợp vào mission planning loop.
...
Smart-city surveillance networks increasingly rely on distributed edge cameras, yet rapid dissemination of high-value recognition payloads remains challenging under strict mission-time constraints. In emergency pursuit scenarios, a UAV can quickly traverse a suspicious area, but each ground node is visible only for a short contact interval. If the UAV stops at each node for full payload delivery, mission latency becomes prohibitive. If it flies continuously, each node receives only partial information. This creates a fundamental trade-off between mobility efficiency and detection reliability.

This work models the UAV as a mobile data ferry that continuously broadcasts payload fragments while following an optimized path. Ground nodes accumulate confidence from partial evidence and trigger intra-cell cooperation to exchange missing fragments before the UAV leaves the region. In contrast to simplified binary-link assumptions, we explicitly model packet-level feasibility through geometry-aware air-to-ground propagation, contact-window validation, and SNR-to-BER-to-PER loss mapping.

Our approach combines (i) confidence accumulation from logical payload fragments, (ii) cooperative fragment exchange among neighboring nodes, and (iii) a Greedy Max-Coverage with Cost (GMC) trajectory heuristic to balance newly covered suspicious nodes and travel cost. The resulting framework targets earlier mission completion under realistic communication uncertainty.

The main contributions are threefold: 1) a fragment-based confidence model with threshold-driven intra-cell cooperation; 2) a mobility-coupled packet-level PHY decision pipeline for dissemination feasibility; and 3) a practical GMC-based planning strategy that integrates candidate centroids and deterministic tie-breaking for reproducible execution. The remainder of the paper is organized as follows: Section II reviews related work, Section III presents the system model, Section IV formulates the optimization problem, Section V details the proposed method, Section VI reports evaluation results, and Sections VII--VIII discuss limitations and conclude.
...
% Mục tiêu setup: đủ thông tin để người khác reproduce được kết quả.
% Nên chia thành 5 nhóm tham số:
% 1) Topology/network:
...
%
% Báo cáo thêm cách chạy:
% - số seeds, CI 95%, runtime budget mỗi scenario.
% - điều kiện dừng mô phỏng (mission complete hoặc timeout T_max).

\subsection{Metrics}
% Define evaluation metrics.
% Metrics nên tách 3 nhóm để khớp đóng góp:
% A. Mission objective metrics:
%    - Time-to-detection T_detect (chính)
%    - Mission completion rate trước deadline
% B. Dissemination/cooperation metrics:
%    - số fragment nhận trực tiếp từ UAV
%    - số fragment nhận qua cooperation
%    - cooperation overhead (bytes/messages/latency)
% C. Trajectory + PHY realism metrics:
%    - total path length, travel time, number of waypoints
%    - packet delivery ratio (PDR), PER-observed, contact-window reject ratio
%
% Lưu ý nhất quán:
% - Nếu termination theo suspicion point n^*, metric chính phải đo theo n^*.
% - Tránh chỉ báo cáo metric trung bình toàn \mathcal{P} nếu objective formal không dùng nó.

\subsection{Results}
% Placeholder for figures/tables and result discussion.
% Cách trình bày kết quả gợi ý:
% 1) Main result table: so sánh các baseline theo T_detect và completion rate.
% 2) Ablation 1: bỏ cooperation để đo lợi ích collaboration.
% 3) Ablation 2: thay đổi alpha/eps hoặc tắt centroid để đo ảnh hưởng GMC.
% 4) Ablation 3: đơn giản hóa PHY (không contact-window) để cho thấy sai lệch khi bỏ realism.
% 5) Sensitivity analysis: sweep K, speed UAV, \tau_coop, \tau_alert.
%
% Khi thảo luận kết quả, nên trả lời trực tiếp 3 câu hỏi:
% - Cách tiếp cận có giảm T_detect so với baseline không?
% - Lợi ích đến từ path planning, cooperation hay cả hai?
% - Kết luận còn giữ khi thay đổi tham số/chế độ kênh không?

...
NS_LOG_COMPONENT_DEFINE("Scenario5BaseStationNode");
...
AssignCellIdAndColorForGroundNodes(double cellRadius)
{
    ...
    const int32_t gridOffset = ::ns3::wsn::scenario5::params::ComputeDefaultHexGridOffset(nodeCount);
...
ValidateIntraCellRoutingTrees()
{
    ...
    for (const auto& [cellId, members] : nodesByCell)
    {
        ...
        for (uint32_t nodeId : members)
        {
            ...
            auto routeIt = ::ns3::wsn::scenario5::params::g_intraCellRoutingTree.find(nodeId);
            if (routeIt == ::ns3::wsn::scenario5::params::g_intraCellRoutingTree.end())
            {
            ...
            for (int32_t neighborCellId : neighborCells)
            {
                ...
                if (::ns3::wsn::scenario5::params::g_cellGatewayPairs.find(cellId) ==
                    ::ns3::wsn::scenario5::params::g_cellGatewayPairs.end() ||
                    ::ns3::wsn::scenario5::params::g_cellGatewayPairs[cellId].find(neighborCellId) ==
                    ::ns3::wsn::scenario5::params::g_cellGatewayPairs[cellId].end() ||
                    ::ns3::wsn::scenario5::params::g_cellGatewayPairs[cellId][neighborCellId].empty())
                {
                ...
                const auto& gateways = ::ns3::wsn::scenario5::params::g_cellGatewayPairs[cellId][neighborCellId];
                ...
                while (hops < maxHops)
                {
                    ...
                    auto routeIt = ::ns3::wsn::scenario5::params::g_intraCellRoutingTree.find(current);
                    if (routeIt == ::ns3::wsn::scenario5::params::g_intraCellRoutingTree.end())
                    {
                        break;
                    }
...
BuildIntraCellRoutingTrees()
{
    ...
    // Clear routing tree
    ::ns3::wsn::scenario5::params::g_intraCellRoutingTree.clear();
    ...
    for (const auto& [cellId, members] : nodesByCell)
    {
        ...
        {
            ::ns3::wsn::scenario5::params::g_intraCellRoutingTree[nodeId][cellId] = parentId;
            totalTreeNodes++;
        ...
        if (::ns3::wsn::scenario5::params::g_cellGatewayPairs.find(cellId) !=
            ::ns3::wsn::scenario5::params::g_cellGatewayPairs.end())
        {
            const auto& neighborGateways = ::ns3::wsn::scenario5::params::g_cellGatewayPairs[cellId];
            ...
            for (const auto& [neighborCellId, gatewayList] : neighborGateways)
            {
                ...
                {
                    ::ns3::wsn::scenario5::params::g_intraCellRoutingTree[nodeId][neighborCellId] = parentId;
                ...
                for (uint32_t memberId : members)
                {
                    auto& routeMap = ::ns3::wsn::scenario5::params::g_intraCellRoutingTree[memberId];
    ...
    if (ns3::wsn::scenario5::params::g_resultFileStream)
    {
        *ns3::wsn::scenario5::params::g_resultFileStream
        ...
        for (const auto& [nodeId, cellRoutes] : ::ns3::wsn::scenario5::params::g_intraCellRoutingTree)
        {            *ns3::wsn::scenario5::params::g_resultFileStream
        ...
        for (int32_t neighborCellId : cellNeighbors[cellId])
        {
            ...
            for (uint32_t nodeId : members)
            {
                ...
                if (closestGateway != nodeId)
                {
                    ...
                    ::ns3::wsn::scenario5::params::g_intraCellRoutingTree[nodeId][neighborCellId] = firstHop;
                    routesAdded++;
...
SelectSuspiciousRegionForBsInit()
{
    ...
    const double targetPercent = ::ns3::wsn::scenario5::params::BS_INIT_SUSPICIOUS_TARGET_PERCENT;
    const uint32_t targetNodeCount = std::max(
        ::ns3::wsn::scenario5::params::BS_INIT_SUSPICIOUS_MIN_TARGET_NODES,
    ...
    const uint32_t MAX_ITERATIONS = ::ns3::wsn::scenario5::params::BS_INIT_SUSPICIOUS_MAX_ITERATIONS;
    ...
    if (ns3::wsn::scenario5::params::g_resultFileStream)
    {
        ...
        *ns3::wsn::scenario5::params::g_resultFileStream
            ...
            << "[SUSPICIOUS-REGION] ";
        for (uint32_t nodeId : suspiciousNodes)
        {            *ns3::wsn::scenario5::params::g_resultFileStream << " " << nodeId;
        }   
        *ns3::wsn::scenario5::params::g_resultFileStream
            << " (Cells: ";
        for (int32_t cellId : suspiciousCells)
        {            *ns3::wsn::scenario5::params::g_resultFileStream << " " << cellId;
        }
        *ns3::wsn::scenario5::params::g_resultFileStream << ")" << std::endl;
    }
...
{
    const uint32_t fragmentCount = ::ns3::wsn::scenario5::params::BS_INIT_FRAGMENT_GENERATION_COUNT;
...
PlanUavFlightPathsForBsInit()
{
    ...
    const double altitude = ::ns3::wsn::scenario5::params::BS_INIT_UAV_PATROL_ALTITUDE;
    const double broadcastRadius = ::ns3::wsn::scenario5::params::UAV_BROADCAST_RADIUS;
    
    // UAV1: Fast speed with hover time
    const double uav1Speed = ::ns3::wsn::scenario5::params::UAV1_SPEED;
    const double uav1HoverTime = ::ns3::wsn::scenario5::params::UAV1_HOVER_TIME;
    
    // UAV2: Slow speed without hover time
    const double uav2Speed = ::ns3::wsn::scenario5::params::UAV2_SPEED;
    const double uav2HoverTime = ::ns3::wsn::scenario5::params::UAV2_HOVER_TIME;
...
SelectCrosscellGatewayPairs(double neighborRadius)
{
    ...
    ::ns3::wsn::scenario5::params::g_cellGatewayPairs.clear();
    ...
    for (const auto& [cellId, neighbors] : cellNeighbors)
    {
        ...
        for (int32_t neighborCellId : neighbors)
        {
            ...
            if (found)
            {
                ::ns3::wsn::scenario5::params::g_cellGatewayPairs[cellId][neighborCellId].push_back(bestGw1);
                ::ns3::wsn::scenario5::params::g_cellGatewayPairs[neighborCellId][cellId].push_back(bestGw2);
                gatewayPairCount++;
...
routing::BaseStationNode::Initialize()
{
    ...
    if (::ns3::wsn::scenario5::params::g_resultFileStream && 
        ::ns3::wsn::scenario5::params::g_resultFileStream->is_open())
    {
        *::ns3::wsn::scenario5::params::g_resultFileStream
            ...
            << std::endl;
        ::ns3::wsn::scenario5::params::g_resultFileStream->flush();
    }

    // Step 1: tính cellId + cellColor cho từng ground node từ position hiện tại
    AssignCellIdAndColorForGroundNodes(::ns3::wsn::scenario5::params::HEX_CELL_RADIUS);
    if (::ns3::wsn::scenario5::params::g_resultFileStream && 
        ::ns3::wsn::scenario5::params::g_resultFileStream->is_open())
    {
        *::ns3::wsn::scenario5::params::g_resultFileStream
            ...
            << "  Cell radius: " << ::ns3::wsn::scenario5::params::HEX_CELL_RADIUS << "m" << std::endl
            << std::endl;
        ::ns3::wsn::scenario5::params::g_resultFileStream->flush();
    }

    // Step 2: neighbor + 2-hop discovery theo bán kính truyền tin
    DiscoverNeighborsAndTwoHopsForGroundNodes(::ns3::wsn::scenario5::params::NEIGHBOR_DISCOVERY_RADIUS);
    if (::ns3::wsn::scenario5::params::g_resultFileStream && 
        ::ns3::wsn::scenario5::params::g_resultFileStream->is_open())
    {
        ...
        *::ns3::wsn::scenario5::params::g_resultFileStream
            ...
            << "  Discovery radius: " << ::ns3::wsn::scenario5::params::NEIGHBOR_DISCOVERY_RADIUS << "m" << std::endl
    ...
    SelectCellLeadersByNearestCellCenter(::ns3::wsn::scenario5::params::HEX_CELL_RADIUS);
    if (::ns3::wsn::scenario5::params::g_resultFileStream && 
        ::ns3::wsn::scenario5::params::g_resultFileStream->is_open())
    {
    ...
    SelectCrosscellGatewayPairs(::ns3::wsn::scenario5::params::NEIGHBOR_DISCOVERY_RADIUS);
    if (::ns3::wsn::scenario5::params::g_resultFileStream && 
        ::ns3::wsn::scenario5::params::g_resultFileStream->is_open())
    {
        uint32_t gatewayPairCount = 0;
        for (const auto& [cellId, neighbors] : ::ns3::wsn::scenario5::params::g_cellGatewayPairs)
        {
        ...
        ::ns3::wsn::scenario5::params::g_resultFileStream->is_open())
    {
        *::ns3::wsn::scenario5::params::g_resultFileStream
            ...
            << "  Total routing entries: " << ::ns3::wsn::scenario5::params::g_intraCellRoutingTree.size() << std::endl
            << std::endl;
        ::ns3::wsn::scenario5::params::g_resultFileStream->flush();
    }
        ...
        ::ns3::wsn::scenario5::params::g_resultFileStream->is_open())
    {
        ...
        *::ns3::wsn::scenario5::params::g_resultFileStream << std::endl << std::endl;
        ::ns3::wsn::scenario5::params::g_resultFileStream->flush();
    }
...
routing::BaseStationNode::SelectSuspiciousRegion()
{
    ...
    std::set<uint32_t> suspiciousNodes =
        SelectSuspiciousRegionFromTopology(m_topology, ::ns3::wsn::scenario5::params::SUSPICIOUS_COVERAGE_PERCENT);
...
routing::BaseStationNode::CalculateFlightPath(const std::set<uint32_t>& suspiciousNodes)
{
    ...
    path = BuildGreedyFlightPath(
        m_topology,
        suspiciousNodes,
        ::ns3::wsn::scenario5::params::UAV_PATH_ALTITUDE,
        ::ns3::wsn::scenario5::params::UAV_PATH_SPEED);
